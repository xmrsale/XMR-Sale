#!/bin/bash
while true; do
    if [[ -f ".bitmonero/wallet" ]]; then
        echo "Found wallet, booting monero-wallet-rpc..."
        monero-wallet-rpc --rpc-bind-ip=0.0.0.0 --rpc-bind-port=18090 --daemon-host monerod --daemon-port 18089 --wallet-file .bitmonero/wallet --password '' --non-interactive
    else
